...
def dashboard(usuario):
    ...
    if not todos_mesoneros:
        ...
        if not historial:
            ...
            df_hist["tipo"] = df_hist["tipo"].map(
                {"error_estandar": "Error estándar", "amonestacion_grave": "Amonestación grave"}
            )
            tabla_hist = df_hist[["fecha", "tipo", "evaluador", "justificacion", "created_at"]].rename(
                columns={"created_at": "hora exacta"}
            )
            st.dataframe(tabla_hist, use_container_width=True, hide_index=True)

            c1, c2 = st.columns(2)
            c1.metric("Total errores estándar (histórico)", int((df_hist["tipo"] == "Error estándar").sum()))
            c2.metric("Total amonestaciones graves (histórico)", int((df_hist["tipo"] == "Amonestación grave").sum()))
