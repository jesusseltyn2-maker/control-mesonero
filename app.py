...
def panel_diario(usuario):
    ...
    if st.button("✅ Guardar y cortar turno", type="primary"):
        ...
        st.success(
            "Turno cerrado. Todos los registros de hoy ya estaban guardados en la nube "
            "en el momento en que los ingresaste; esto solo deja constancia en el rastro "
            "de auditoría de que el turno terminó."
        )
...
def dashboard(usuario):
    ...
    evaluaciones = (
        supabase.table("evaluaciones")
        .select("*, mesoneros(nombre_completo)")
    ...
    with st.expander("Ver detalle completo (todas las justificaciones)"):
        detalle = df[["fecha", "mesonero", "tipo", "justificacion"]].sort_values("fecha", ascending=False)
